...
datatable(female_rent, options=list(lengthMenu = c(3,10,30)), 
          extensions="Responsive",
          caption = 'Table 2: Female Housing dataset')
```

```{r numerical, warning=FALSE}
pander(Stephanie, caption = "Table 3: Stephanie's Question")
```

&emsp; The Apartment I would recommend given the data from the table above is Birch Plaza. It fits all you are looking for in an apartment. Birch Plaza has a capacity of 343 which allows you to meet new people. It is .062 miles from campus making it one of the closest apartments to campus. The monthly cost is 280 dollars which is the cheapest cost per month of apartments in Rexburg. 

This is a picture of Birch Plaza. You can literally see the BYUI Stadium from the parking lot.

![](https://www.liveherehousing.com/photos/n1077.jpg)

This vizualization show Birch Plaza as a circle and other apartments as triangles. This graph helps show apartments with respect to distance and cost. We can see that visually Birch Plaza is the best compared to other apartments. 

```{r viz, message=FALSE, warning=FALSE, paged.print=FALSE}
Stephanie %>% mutate(apartmentforyou = ifelse(Apartment == "BIRCH PLAZA", "Birch Plaza", "Other")) %>% 
  ggplot() + 
