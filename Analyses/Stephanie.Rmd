...
```{r}
female_rent <- Rent %>% 
  select(- UtilityDeposit, - Address, - Phone, - FloorPlans, - Website, - Description, - ParkingStalls, - Latitude, - Longitude) %>% # Unneeded variables
  filter(Gender == "F") %>% select(- Gender) %>%  # filter to female housing
  select(- PrivateRoomPrice) %>% # Remove private room -- She said she would like to meet people
  mutate(MonthlyCost = Price/3.5) #Find rent per month
```
