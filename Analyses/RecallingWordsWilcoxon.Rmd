...
wilcox.test(correct ~ condition, data = friendly, mu = 0, alternative = "greater", paired = TRUE, conf.level = 0.95) %>% 
    pander()
t.test(correct ~ condition, data = friendly, mu = 0, alternative = "greater", paired = TRUE, conf.level = 0.95) %>% 
    pander()
```

According to the Wilcoxon test my hypothesis was correct that using the meshed method is better compared to the before method. The t-test also proves the hypothesis, the t-test gives about the same result values as the Wilcoxon test.
